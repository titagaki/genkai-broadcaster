...
- 最終更新: 2026-09-13
- 状態: 実装済み・実機確認済み。v0.2.0 (2026-09-13) で公開。提供側は JPNKN Vox v0.3.0
  (`AndroidStudioProjects/JPNKNVox`、`docs/spec/comment-export-spec.md`)。未確認の項目は §5 に残してある
...
2026-09-13 に JPNKN Vox v0.3.0 (debug 同士) で確認済み:

- `<queries>` 宣言だけで提供側が列挙され、設定で選べること。
- 配信中にコメントが右下 / 右上に乗り、10 秒で消えること。投稿者名の前置。
- 縦配信でフィルタの Bitmap が出力寸法 (720×1280) に一致し、上下左右が正しいこと (文字サイズは短辺基準に修正)。
- 読み上げ停止中は状態行が `JPNKN Vox が停止中`、開始で `JPNKN Vox` に変わること。
- 提供側の自前権限は定義側が後入れだと付与されない → 権限を撤去 (§3.3)。

未確認:

- 配信中に提供側を強制終了 → OS の再起動で復帰すること。配信は途切れないこと。

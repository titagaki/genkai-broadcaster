...
UI変更は、[ui-current.md](ui-current.md) で現状を確認し、[ui-design.md](ui-design.md) を合意してから実装する。
参考アプリの調査結果は [ui-references.md](ui-references.md) を参照。
...
- 前面・背面・別レンズ、ミュート状態で設定往復と画面再生成を試し、映像・ミュート・経過時間が保持されることを確認する。
- 縦横両画面で文字サイズを拡大し、操作パネルをスクロールでき、設定・停止ボタンを押せることを確認する。

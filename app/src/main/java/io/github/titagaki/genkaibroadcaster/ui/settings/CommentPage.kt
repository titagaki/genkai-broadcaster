...
/**
 * コメントページ: 映像に載せるコメントの提供アプリと表示位置を選ぶ。
 ...
 * @param enabled 提供アプリの選択可否 (配信中は不可)。表示位置は配信中でも変えられる
 */
@Composable
internal fun CommentPage(
    ...
    onPositionChange: (CommentPosition) -> Unit
) {
    SettingsSection("コメントの取得元", "配信停止中のみ変更できます") {
        ...
        Text(
            if (sources.isEmpty()) {
                ...
                "配信中だけ提供アプリに接続し、届いたコメントを映像に最新 ${StreamConfig.COMMENT_MAX_LINES} 件、" +
                    "${StreamConfig.COMMENT_DISPLAY_MS / 1000} 秒ずつ表示します。プレビューにも同じものが映ります。"
            },
            style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant
        )
    }
    SettingsSection("表示位置", "配信中も変更でき、即時反映されます") {
        RadioOptionList(
            options = CommentPosition.entries.map { it to "映像の右${it.label}" },
            selected = position,
            enabled = true,
